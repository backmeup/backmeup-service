...
public class SerializiationTest {
	...
	public void testBackupJobSerializiation() {
		BackMeUpUser user = new BackMeUpUser(1L, "john.doe", "John", "Doe",
				"Sepp@Mail.at", "John123!#");
		Profile source = new Profile(2L, user, "TestProfile",
				"org.backmeup.source", PluginType.Source);
		ProfileOptions options = new ProfileOptions(source, new String[] {
				"folder1", "folder2" });
		Profile sink = new Profile(2L, user, "TestProfile2",
				"org.backmeup.sink", PluginType.Sink);
		List<ActionProfile> actions = new ArrayList<>();
		BackupJob job = new BackupJob(user, options, sink, actions, new Date(),
				new Date().getTime() + 1000000L, "TestJob1", false);
		...
		for (int i = 0; i < job.getRequiredActions().size(); i++) {
			...
			Iterator<ActionProfile> apIt2 = restored.getRequiredActions()
					.iterator();
			while (apIt1.hasNext()) {
	...
	@Test
	public void testBackupJobSerializiationOfObjects() {
		BackMeUpUser user = new BackMeUpUser(1L, "john.doe", "John", "Doe",
				"Sepp@Mail.at", "John123!#");
		Profile source = new Profile(2L, user, "TestProfile",
				"org.backmeup.source", PluginType.Source);
		...
		Assert.assertEquals("BackUpJob serialization -> deserialization issue",
				job, restored);

	}

	@Test
	public void testBackUpJobEqualityOfObjects() {
		BackMeUpUser user = new BackMeUpUser(1L, "john.doe", "John", "Doe",
				"Sepp@Mail.at", "John123!#");
		Profile source = new Profile(2L, user, "TestProfile",
				"org.backmeup.source", PluginType.Source);
		...
		// the objects equal methods have been overwritten - the logic
		// of testProfiles, testUser, testActionProfile, testProfileOptions
		// should be dealth with there
		//
		Assert.assertEquals("equals should have been properly overwritten",
				job, restored);
	}
