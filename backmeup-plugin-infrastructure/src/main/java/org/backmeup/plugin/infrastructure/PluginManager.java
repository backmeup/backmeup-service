...
public class PluginManager {
    ...
    public <T> T service(final Class<T> service, final String pluginId) {
        ...
        return (T) Proxy.newProxyInstance(PluginManager.class.getClassLoader(),
            ...
            public Object invoke(Object o, Method method, Object[] os) throws Throwable {
                ...
                Object instance = getBundleContext().getService(serviceRef);
                Object ret = null;
                try {
        ...
        return new Iterable<T>() {
            ...
            public Iterator<T> iterator() {
                try {
                    ...
                    ServiceReference[] refs = getBundleContext().getServiceReferences(service.getName(), filter);
                    if (refs != null) {
                        for (ServiceReference s : refs) {
                            services.add((T) Proxy.newProxyInstance(
                                    PluginManager.class.getClassLoader(),
                                    new Class[] { service },
                                    new SpecialInvocationHandler(getBundleContext(), s)));
                        }
                    }
    ...
    private <T> ServiceReference getReference(final Class<T> service, final String pluginId) {
        ...
        if (pluginId == null) {
            ref = getBundleContext().getServiceReference(service.getName());
        } else {
            ServiceReference[] refs;
            String filter = String.format(PLUGIN_FILTER_FORMAT, pluginId);
            try {
                refs = getBundleContext().getServiceReferences(service.getName(), filter);
    ...
    private void startDeploymentMonitor() {
        this.deploymentMonitor = new DeploymentMonitor(getBundleContext(), deploymentDirectory);
